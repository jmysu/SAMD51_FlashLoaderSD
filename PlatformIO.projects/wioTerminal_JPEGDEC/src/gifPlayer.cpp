...
extern LGFX lgfxTFT;
#define DISPLAY_WIDTH lgfxTFT.width()
...
const int maxLoopsDuration = 5500;  // ms, max cumulated time after the GIF will break loop
const int maxGifDuration = 6000;    // ms, max GIF duration
...
static void lgfxTFTDraw(int x, int y, int w, int h, uint16_t *lBuf) {
    lgfxTFT.pushRect(x + xOffset, y + yOffset, w, h, lBuf);
}

// Draw a line of image directly on the lgfxTFT
void GIFDraw(GIFDRAW *pDraw) {
    ...
    s = pDraw->pPixels;
    /* Comment these lines to enable the background transparency
    ...
    if (pDraw->ucHasTransparency) {  // if transparency used
        ...
        while (x < iWidth) {
            ...
            if (iCount) {  // any opaque pixels?
                lgfxTFTDraw(pDraw->iX + x, y, iCount, 1, (uint16_t *)usTemp);
                x += iCount;
                iCount = 0;
            }
        ...
        lgfxTFTDraw(pDraw->iX, y, iWidth, 1, (uint16_t *)usTemp);
    }
} /* GIFDraw() */
...
int gifPlay(char *gifPath) {  // 0=infinite
    ...
    xOffset = (lgfxTFT.width() - w) / 2;
    yOffset = (lgfxTFT.height() - h) / 2;

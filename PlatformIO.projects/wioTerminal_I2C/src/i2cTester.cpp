...
void loop() {
    ...
    for (address = 1; address < 127; address++) {
        ...
        //if (error == 0) {
        Wire1.beginTransmission(address+1); // Special action for SAMD Series
        if (error == 0 && Wire1.endTransmission() != 0 ) { // Special flag for SAMD Series
